...
import {
  ...
  loadDb,
  removeMember as removeMemberLocal,
  type MembersDb,
} from '@/utils/membership-store'
...
export default function AdminPage() {
  ...
  const enrollViaPubkey = async () => {
    setSubmitting(true)
    try {
      const { db: nextDb, member } = await enrollMember({
        name: name.trim(),
      ...
      setDb(nextDb)
      Add(`Enrolled ${member.name} at index ${member.leaf_index}`, { type: 'success' })
      setName('')
      setPubkeyHex('')
    } catch (err) {
  ...
  const enrollViaSignature = async () => {
    ...
    try {
      ...
      const { db: nextDb, member } = await enrollMemberFromSignature({
        name: message.name,
  ...
  const removeMember = async (index: number, memberName: string) => {
    ...
    try {
      const { db: nextDb, removed } = await removeMemberLocal(index)
      setDb(nextDb)
      Add(`Removed ${removed.name}`, { type: 'success' })
    } catch (err) {
  ...
  const downloadCard = async (index: number, name: string) => {
    try {
      const card = await cardForIndex(index)
  ...
  return (
    <div className='flex-column align-center'>
      <h1 className='text-xl'>Create commitment · Member enrollment</h1>
      <p className='mt-2 text-sm opacity-80'>
        Enroll a member by HaLo tap, by wallet signature, or by raw pubkey. Each enrollment commits the
        member&apos;s public key as a leaf and recomputes the Merkle root. State is persisted to{' '}
        <code>localStorage</code> under <code>bordel-membership-tree-v1</code> — the verify page reads the
        same key.
      </p>
      <p className='mt-2 text-xs opacity-60'>
        Local-only: nothing leaves this browser. Clear site data to wipe the tree.
      </p>
      ...
      <div className='mt-6 w-full grid gap-3'>
        ...
        {mode === 'wallet' && (
          <div className='form-control w-full'>
            <div className='label'>
              <span className='label-text'>Claimed address</span>
            </div>
            <input
              ...
              onChange={(e) => {
                ...
                setClaimedAddress(next)
                setClaimedAddressIsValid(isAddress(next))
              }}
              disabled={submitting}
            />
            ...
            <div className='text-xs mt-1'>
              ...
              {claimedChecksum ? (
                ...
                <span className='opacity-50'>— (type an address or use the connected wallet)</span>
              )}
            </div>
            {addressMismatch && (
