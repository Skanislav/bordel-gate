...
import { leafFromPubkeyBytes, pathFor } from '@/utils/merkle'
...
export default function VerifyPage() {
  ...
  const runProof = async (method: Method) => {
    ...
    setErrorMsg(null)
    if (!cache) {
      Add('no member tree cached locally — create a commitment via /commit', { type: 'error' })
      return
    }
    if (cache.members_count === 0) {
      Add('no members enrolled — visit /commit', { type: 'error' })
      return
    }
    ...
    try {
      ...
      if (leafIndex < 0) {
        throw new Error(
          `not enrolled (leaf ${truncate(leafHex)} not in ${cache.members_count}-member tree). enroll at /commit.`,
        )
      }
  ...
  return (
    <div className='text-[var(--bordel-fg)] py-4'>
      {/* status row: breadcrumb left, local member count right */}
      <div className='flex justify-between items-center text-xs opacity-60'>
        <span>&gt; / gate / index.cgi</span>
        <span>online: {String(onlineCount).padStart(2, '0')}</span>
      </div>
